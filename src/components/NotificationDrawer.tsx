...
export const NotificationDrawer: React.FC<NotificationDrawerProps> = ({
  ...
  const handleTestPush = async () => {
    ...
    const ok = await sendTestWebPushNotification();
    if (ok) {
      setTestStatus('✓ Đã gửi thông báo!');
    } else {
      setTestStatus('Chưa gửi được');
    }
    setTimeout(() => setTestStatus(null), 3500);
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
          ...
          >
            ...
            {/* Web Push Notification Control Bar */}
            {pushPermission !== 'unsupported' && (
              <div className="border-b border-[#e5e7eb] text-xs">
                {pushPermission === 'default' && (
                  <div className="p-3.5 bg-[#fdf2f7] flex items-start justify-between gap-3">
                    <div className="flex items-start gap-2.5 min-w-0">
                      ...
                      <div>
                        <div className="font-title font-bold text-[#78234a] text-xs flex items-center gap-1.5">
                          <span>Thông báo trình duyệt (Web Push)</span>
                          <span className="text-[10px] font-ui px-1.5 py-0.2 rounded bg-[#963861] text-white font-semibold">Mới</span>
                        </div>
                        <p className="text-[11px] font-ui text-[#8a335a] mt-0.5 leading-relaxed">
                          Nhận thông báo ngay trên màn hình khi bạn được giao việc hoặc khi dự án có cập nhật khẩn cấp.
                        </p>
                      </div>
                    </div>
                    <button
                      ...
                      className="px-3 py-1.5 rounded-[6px] bg-[#963861] text-white hover:bg-[#78234a] font-ui font-semibold text-xs transition-colors shrink-0 shadow-2xs cursor-pointer"
                    >
                      Bật ngay
                    </button>
                  </div>
                ...
                {pushPermission === 'granted' && (
                  <div className="px-4 py-2.5 bg-[#f8fafc] flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className={`w-2 h-2 rounded-full shrink-0 ${isPushEnabled ? 'bg-[#16a34a] animate-pulse' : 'bg-[#94a3b8]'}`} />
                      <span className="font-ui text-xs font-medium text-[#334155] truncate">
                        Thông báo đẩy Web Push: <strong className={isPushEnabled ? 'text-[#15803d]' : 'text-[#64748b]'}>{isPushEnabled ? 'Đang bật' : 'Đã tắt'}</strong>
                      </span>
                      {testStatus && (
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {isPushEnabled && (
                        <button
                          ...
                          className="px-2 py-1 text-[11px] font-ui text-[#1d508d] bg-white border border-[#cbd5e1] hover:bg-[#f1f5f9] rounded-[4px] font-medium transition-colors cursor-pointer flex items-center gap-1"
                      ...
                      <label className="relative inline-flex items-center cursor-pointer" title={isPushEnabled ? 'Tắt thông báo Web Push' : 'Bật thông báo Web Push'}>
                        <input
                          type="checkbox"
                ...
                {pushPermission === 'denied' && (
                  <div className="p-3 bg-[#fffbeb] text-[#92400e] flex items-start gap-2">
                    <AlertTriangle className="w-3.5 h-3.5 text-[#d97706] shrink-0 mt-0.5" />
                    <div className="text-[11px] leading-relaxed">
                      <strong>Trình duyệt đang chặn thông báo:</strong> Nhấn vào biểu tượng ổ khóa 🔒 trên thanh địa chỉ URL và chọn <em>Cho phép thông báo (Allow)</em> để nhận Web Push.
                    </div>
                  </div>
                )}
              </div>
